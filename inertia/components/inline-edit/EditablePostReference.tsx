...
export function EditablePostReference({ moduleId, path, multiple, postType, label }: EditablePostReferenceProps) {
	...
	const normalize = (val: ValueShape): string[] => {
		...
		if (Array.isArray(val)) {
			return val
				.map((v) => (typeof v === 'string' ? v : v.id || v.slug || ''))
				.filter(Boolean)
		}
	...
	const labelForId = (id: string) => {
		...
		if (fromMeta) return `${fromMeta.title} (${fromMeta.type}${fromMeta.locale ? ` · ${fromMeta.locale}` : ''})`
		const hit = options.find((o) => o.id === id)
					...
					{selectedIds.map((id) => (
						...
						>
							...
							<button
								...
								onClick={() =>
									multiple
										? setValue(moduleId, path, selectedIds.filter((x) => x !== id))

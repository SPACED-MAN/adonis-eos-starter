...
export function SidebarTrigger({ className, children }: { className?: string; children?: React.ReactNode }) {
	...
	return (
		<button
			...
			className={cn(
				'inline-flex items-center rounded p-2 hover:bg-backdrop-medium text-standout',
				className
			)}
		>
...
export function SidebarContent({ children, className }: { children: React.ReactNode; className?: string }) {
	return <div id="admin-sidebar" className={cn('h-full overflow-y-auto', className)}>{children}</div>
}

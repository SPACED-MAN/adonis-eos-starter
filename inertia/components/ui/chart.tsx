  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-line-low bg-backdrop px-2.5 py-1.5 text-xs shadow-xl",
          className
        )}
      >
          ...
          {payload.map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={cn(
                  "flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-neutral-low",
                ...
                {formatter && item?.value !== undefined && item.name !== undefined ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      {item.value && (
                        <span className="font-mono font-medium tabular-nums text-neutral-high">
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              "flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-neutral-low"
            )}
          >
...
function getPayloadConfigFromCustomKey(
  ...
  const configLabelKey = item[key as keyof typeof item] || item.dataKey || key

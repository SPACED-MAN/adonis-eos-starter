...
export default function Share({
  ...
  const containerClass = `w-full ${padding} ${backgroundColor}`
  ...
  const content = (
    <div className={`flex flex-wrap items-center ${alignmentClass} gap-4`}>
      {title && (
        <span className="text-sm font-bold uppercase tracking-wider text-neutral-medium mr-2">
          {title}
        </span>
      )}
        ...
        {enabledNetworks.map((n, idx) => {
          const item = (
            <a
              ...
              className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-backdrop-low border border-line-low text-neutral-medium hover:text-standout-high hover:border-standout-medium hover:shadow-sm transition-all duration-200 group"

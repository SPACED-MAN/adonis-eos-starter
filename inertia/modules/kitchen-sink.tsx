      ...
      <div className="border border-line-low rounded-lg bg-backdrop-low p-6 sm:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <Info
            ...
            dataPath="category"
            type="select"
            dataOptions='["general","news","updates"]'
          />
          <Info
            label="Tags"
            ...
            dataPath="tags"
            type="multiselect"
            dataOptions='["alpha","beta","gamma"]'
            dataMulti="true"
          />
          <Info
            label="Featured"
            value={props.featured ? 'Yes' : 'No'}
            dataPath="featured"
            type="boolean"
          />
          <Info
            ...
            dataPath="publishDate"
            type="date"
          />
          <Info
            label="Link URL"
            value={typeof props.linkUrl === 'string' ? props.linkUrl : (props.linkUrl?.url ?? '')}
            dataPath="linkUrl"
            type="link"
          />
          ...
          <Info
            ...
            dataPath="selectOption"
            type="select"
            dataOptions='["general","news","updates"]'
          />
          <Info
            label="Multi Select"
            value={(props.multiSelect || []).join(', ')}
          ...
          <Info
            label="Boolean Flag"
            value={props.booleanFlag ? 'Yes' : 'No'}
            dataPath="booleanFlag"
            type="boolean"
          />
          <Info
            label="Number Field"
            value={String(props.numberField ?? '')}
        ...
        {Array.isArray(props.postRefs) && (
          <div className="text-sm space-y-1">
            ...
            <div className="text-neutral-medium">
              Selected IDs: {(props.postRefs || []).join(', ') || '(none)'}
            </div>
          </div>
        )}
            ...
            <ul className="list-disc pl-5">
              {props.items.map((it, idx) => (
                ...
                >
                  ...
                  <span
                    className={it.highlight ? 'text-standout-high' : 'text-neutral-high'}
                    data-inline-path={`items.${idx}.value`}
                  >
                    {it.value}
                  </span>{' '}
                  <span

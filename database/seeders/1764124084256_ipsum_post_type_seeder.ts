...
export default class extends BaseSeeder {
  ...
  public async run() {
    const now = new Date()
    const templateName = 'ipsum-default'
    ...
    if (!existingTemplate) {
      await db.table('templates').insert({
        name: templateName,
        post_type: 'ipsum',
        description: 'Default template for ipsum',
        locked: false,
        created_at: now,
        updated_at: now,
      })
    ...
    const existing = await db.from('url_patterns').where('post_type', 'ipsum').select('locale')

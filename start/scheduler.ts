...
export async function publishDueScheduledPosts() {
  ...
  await db.transaction(async (trx) => {
    await trx
      .from('posts')
      .whereIn('id', ids)
      .update({ status: 'published', published_at: when, updated_at: when } as any)
  })
  // Fire-and-forget activity logs
  for (const id of ids) {
    try {
      await activity.log({
        action: 'post.publish.auto',
        entityType: 'post',
        entityId: id,
        metadata: { reason: 'scheduled', at: when.toISOString() },
      })
    } catch {}
  }
}
...
const isAceCli =
  Array.isArray(process.argv) && process.argv.some((arg) => arg.includes('/ace') || arg === 'ace')

...
export default class MaintenanceMiddleware {
	async handle(ctx: HttpContext, next: NextFn) {
		...
		if (path.startsWith(adminPath()) || path.startsWith('/api/database') || path.startsWith('/api/site-settings')) {
			return next()
		}
		...
		if (settings.isMaintenanceMode) {
			...
			return response.send(await inertia.render('site/MaintenanceMode', {

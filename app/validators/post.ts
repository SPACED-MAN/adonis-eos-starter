...
export const createPostValidator = vine.compile(
	vine.object({
		...
		slug: vine.string().trim().minLength(1).maxLength(255).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/),
		title: vine.string().trim().minLength(1).maxLength(500),
		status: vine.enum(['draft', 'review', 'scheduled', 'published', 'private', 'protected', 'archived']).optional(),
...
export const updatePostValidator = vine.compile(
	vine.object({
		slug: vine.string().trim().minLength(1).maxLength(255).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).optional(),
...
export const bulkActionValidator = vine.compile(
	vine.object({
		action: vine.enum(['publish', 'draft', 'archive', 'delete', 'duplicate', 'regeneratePermalinks']),

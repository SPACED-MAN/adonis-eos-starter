...
export default class ApproveReviewDraft {
  static async handle({ postId, userId }: ApproveReviewDraftParams): Promise<void> {
    ...
    if (!rd) {
      throw new Error('No review draft to approve')
    }

    // 1. Promote basic fields using UpdatePost action
    ...
    // 4. Promote taxonomy term assignments
    if (Array.isArray(rd.taxonomyTermIds)) {

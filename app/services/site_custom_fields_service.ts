...
class SiteCustomFieldsService {
  ...
  async upsertValues(values: Record<string, SiteCustomFieldValue>): Promise<void> {
    ...
    for (const [slug, value] of Object.entries(values)) {
      ...
      const jsonbValue =
        value === null || value === undefined ? null : db.raw('?::jsonb', [JSON.stringify(value)])
      if (exists) {
        await db
          .from('site_custom_field_values')
          .where('field_slug', slug)
          .update({ value: jsonbValue as any, updated_at: now } as any)
      } else {
        await db.table('site_custom_field_values').insert({
          field_slug: slug,

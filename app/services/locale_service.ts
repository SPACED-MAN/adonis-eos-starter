...
export class LocaleService {
  private async hasLocalesTable(): Promise<boolean> {
    try {
      ...
      if (
        e &&
        (e.code === '42P01' ||
          String(e.message || '').includes('relation "locales" does not exist'))
      ) {
        return false
      }
  ...
  async ensureFromEnv(): Promise<void> {
    ...
    await db.from('locales').whereNotIn('code', uniq).update({ is_enabled: false, updated_at: now })
  ...
  private async detectFromHeaders(ctx: HttpContext): Promise<string | null> {
    ...
    const languages = acceptLanguage
      ...
      .map((lang) => {
        ...
        const q = qRaw ? Number.parseFloat(qRaw) : 1.0

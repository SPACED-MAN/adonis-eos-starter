...
class SitemapService {
	...
	async generate(options: { protocol: string; host: string }): Promise<string> {
		...
		const rows: PostRow[] = rowsRaw.map((r: any) => ({
			...
			updatedAt: r.updatedAt ? (r.updatedAt instanceof Date ? r.updatedAt : new Date(r.updatedAt)) : null,
			publishedAt: r.publishedAt ? (r.publishedAt instanceof Date ? r.publishedAt : new Date(r.publishedAt)) : null,
		...
		for (const row of indexable) {
			...
			const fallback = row.locale === defaultLocale ? `/${row.type}/${seg}` : `/{locale}/${row.type}/${seg}`
			try {
		...
		const entries = ordered.map((row) => {
			...
			const loc = locRaw.startsWith('http') ? locRaw : `${options.protocol}://${options.host}${locRaw}`
